...
export class MainScene extends Scene {
	...
	darkBg!: Graphics;
	constructor(isUnli : boolean) {
		let textures: any = [];

		CurrentGameData.isUnlimited = isUnli;
		...
		this.currentLevel.createBoard(CurrentGameData.isUnlimited);
		...
		const exitButton = new Sprite(Globals.resources.reject.texture);
        exitButton.position.y = 35;
        exitButton.position.x = 35;

		exitButton.anchor.set(0.5);
        exitButton.scale.set(0.25*config.minScaleFactor);
        
        exitButton.interactive = true;
        exitButton.buttonMode = true;
 
        exitButton.on("pointerdown",()=>{this.bgMusic.stop();  CurrentGameData.gameStarted = false; SceneManager.instance!.start(new GameMenuScene);})
